...
async function sendCurrentStateToArduino(): Promise<void> {
  try {
    ...
    const stateMessage = `STATE current=${currentNumber};total=${totalTickets}`;
    sendToArduino(stateMessage);
    console.log(`✅ Đã gửi state: current=${currentNumber}, total=${totalTickets}`);
...
async function syncStateToArduino(currentNumber: number, nextNumber: number): Promise<void> {
  ...
  console.log(`🔄 Đồng bộ state → Arduino: current=${currentNumber}, total=${totalTickets}`);
  ...
  supabase
    ...
    .subscribe((status: string) => {
      if (status === 'SUBSCRIBED') {
        console.log('✅ Đã subscribe realtime Supabase');
      }
    });
}
// ============ INITIALIZATION ============
